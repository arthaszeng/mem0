...
async def reembed_missing_vectors(
    ...
    for mid in missing_ids:
        ...
        payload: dict = {
            ...
            "source_app": "memverse",
            "mcp_client": m.app.name if m.app else "memverse",
        }
        if m.project_id:
...
def _normalize_memory_record(m: dict) -> dict:
    ...
    m.setdefault("app_name", m.get("app") or "memverse")
...
async def import_backup(
    ...
    # --- Ensure default app ---
    default_app = db.query(App).filter(App.owner_id == user.id, App.name == "memverse").first()
    if not default_app:
        default_app = App(owner_id=user.id, name="memverse", is_active=True, metadata_={})
        db.add(default_app)
        db.commit()
