"""
Memory client utilities for Memverse.
...
- CRITICAL: Every fact starts with a [Domain] prefix like [OSMP会议管理] or [Memverse]. You MUST preserve the [Domain] prefix in ALL output. Never drop it.
    ...
    try:
        ...
        try:
            ...
            if db_config:
                ...
                memverse_cfg = json_config.get("memverse") or json_config.get("openmemory")
                if memverse_cfg and memverse_cfg.get("custom_instructions"):
                    db_custom_instructions = memverse_cfg["custom_instructions"]
        ...
        try:
            memverse_cfg = db_json_config.get("memverse") or db_json_config.get("openmemory") or {}
            ct = memverse_cfg.get("confidence_threshold")
            if ct is not None:
