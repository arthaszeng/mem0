...
def get_authenticated_user(request: Request, db: Session = Depends(get_db)) -> AuthenticatedUser:
    ...
    Auto-provisions Memverse User on first access."""
    ...
    if not db_user:
        ...
        default_app = App(name="memverse", owner_id=db_user.id)
        db.add(default_app)
        ...
        db.commit()
        db.refresh(db_user)
        logger.info("Auto-provisioned Memverse user: %s", auth_username)
    else:
