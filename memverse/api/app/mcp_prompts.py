...
def register_prompts(mcp, user_id_var, client_name_var, project_slug_var=None):
    ...
    @mcp.prompt(
        ...
        description="Load the current custom instructions configured in Memverse. "
    ...
    async def custom_instructions() -> str:
        ...
        try:
            ...
            if config and config.value:
                instructions = (config.value.get("memverse") or config.value.get("openmemory") or {}).get("custom_instructions")

            from app.utils.prompts import build_fact_extraction_prompt, build_categorization_prompt

            lines = ["# Memverse Instructions\n"]
