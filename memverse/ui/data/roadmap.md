...
- [x] **OpenClaw memverse 插件** — 基于 mem0ai JS SDK 的 OpenClaw 插件，实现 auto-capture 和 auto-recall
...
- [x] **ChatGPT Custom GPT** — OpenAPI Schema 定义 Actions，ChatGPT 直接读写 Memverse
...
- [x] **Concierge 记忆同步** — concierge_chat/search 交互自动同步到 Memverse（source_app=Concierge，infer=True，异步不阻塞）
...
## v2.0 | Memverse Rebrand | completed | 2026-03 | sparkles

项目从 OpenMemory / mem0 品牌全面重命名为 Memverse

- [x] **目录重构** — `openmemory/` → `memverse/`，所有代码引用同步更新
- [x] **Docker 镜像重命名** — 镜像前缀 `mem0/` → `memverse/`，服务名 `openmemory-*` → `memverse-*`
- [x] **URL 路径迁移** — MCP 端点 `/memory-mcp/` → `/memverse-mcp/`
- [x] **环境变量统一** — `MEM0_USER` → `MEMVERSE_USER`，`OPENMEMORY_URL` → `MEMVERSE_URL`
- [x] **API 品牌更新** — FastAPI 标题、MCP 服务名、source_app 默认值、配置键名全部更新
- [x] **UI 品牌更新** — 页面标题、Logo alt、安装说明、设置面板全部更新为 Memverse
- [x] **客户端同步** — OpenClaw 插件、LangGraph Agent 的品牌引用和环境变量同步
- [x] **文档与规则** — Cursor rules、GPT schemas、README、doc/ 全面更新
- [x] **向后兼容** — 配置读取保留 `openmemory` 旧键兼容，source-app 保留旧映射
