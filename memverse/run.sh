...
echo "🚀 Starting Memverse installation..."
...
if [ -z "$OPENAI_API_KEY" ]; then
  echo "❌ OPENAI_API_KEY not set. Please run with: curl -sL https://raw.githubusercontent.com/mem0ai/mem0/main/memverse/run.sh | OPENAI_API_KEY=your_api_key bash"
...
# Function to create compose file by merging vector store config with memverse-mcp service
create_compose_file() {
  ...
  tail -n +2 "$compose_file" | sed '/^volumes:/,$d' | sed "s/memverse_storage/${volume_name}/g" | sed "s/memverse_store/memverse_store/g" >> docker-compose.yml
  
  # Add a newline to ensure proper YAML formatting
  echo "" >> docker-compose.yml
  
  # Add the memverse-mcp service
  cat >> docker-compose.yml <<EOF
  memverse-mcp:
    image: memverse/memverse-mcp:latest
    environment:
      - OPENAI_API_KEY=${OPENAI_API_KEY}
      - USER=${USER}
EOF
  ...
  case "$vector_store" in
    weaviate)
      cat >> docker-compose.yml <<EOF
      - WEAVIATE_HOST=memverse_store
      - WEAVIATE_PORT=8080
EOF
      ;;
    redis)
      cat >> docker-compose.yml <<EOF
      - REDIS_URL=redis://memverse_store:6379
EOF
      ;;
    pgvector)
      cat >> docker-compose.yml <<EOF
      - PG_HOST=memverse_store
      - PG_PORT=5432
      - PG_DB=mem0
      - PG_USER=mem0
      - PG_PASSWORD=mem0
EOF
    ...
    qdrant)
      cat >> docker-compose.yml <<EOF
      - QDRANT_HOST=memverse_store
      - QDRANT_PORT=6333
EOF
      ;;
    chroma)
      cat >> docker-compose.yml <<EOF
      - CHROMA_HOST=memverse_store
      - CHROMA_PORT=8000
EOF
      ;;
    milvus)
      cat >> docker-compose.yml <<EOF
      - MILVUS_HOST=memverse_store
      - MILVUS_PORT=19530
EOF
      ;;
    elasticsearch)
      cat >> docker-compose.yml <<EOF
      - ELASTICSEARCH_HOST=memverse_store
...
EOF
  ...
  # Add common memverse-mcp service configuration
...
install_vector_store_packages() {
  ...
  case "$vector_store" in
    qdrant)
      docker exec memverse-memverse-mcp-1 pip install "qdrant-client>=1.9.1" || echo "⚠️ Failed to install qdrant packages"
      ;;
    chroma)
      docker exec memverse-memverse-mcp-1 pip install "chromadb>=0.4.24" || echo "⚠️ Failed to install chroma packages"
      ;;
    weaviate)
      docker exec memverse-memverse-mcp-1 pip install "weaviate-client>=4.4.0,<4.15.0" || echo "⚠️ Failed to install weaviate packages"
      ;;
    faiss)
      docker exec memverse-memverse-mcp-1 pip install "faiss-cpu>=1.7.4" || echo "⚠️ Failed to install faiss packages"
      ;;
    pgvector)
      docker exec memverse-memverse-mcp-1 pip install "vecs>=0.4.0" "psycopg>=3.2.8" || echo "⚠️ Failed to install pgvector packages"
      ;;
    redis)
      docker exec memverse-memverse-mcp-1 pip install "redis>=5.0.0,<6.0.0" "redisvl>=0.1.0,<1.0.0" || echo "⚠️ Failed to install redis packages"
      ;;
    elasticsearch)
      docker exec memverse-memverse-mcp-1 pip install "elasticsearch>=8.0.0,<9.0.0" || echo "⚠️ Failed to install elasticsearch packages"
      ;;
    milvus)
      docker exec memverse-memverse-mcp-1 pip install "pymilvus>=2.4.0,<2.6.0" || echo "⚠️ Failed to install milvus packages"
      ;;
    *)
...
for i in {1..30}; do
  if docker exec memverse-memverse-mcp-1 python -c "import sys; print('ready')" >/dev/null 2>&1; then
    break
  fi
  sleep 1
done
...
if [ "$VECTOR_STORE" = "milvus" ]; then
  ...
  curl -fsS -X PUT "${NEXT_PUBLIC_API_URL}/api/v1/config/mem0/vector_store" \
    ...
    -d "{\"provider\":\"milvus\",\"config\":{\"collection_name\":\"memverse\",\"embedding_model_dims\":${EMBEDDING_DIMS},\"url\":\"http://memverse_store:19530\",\"token\":\"\",\"db_name\":\"\",\"metric_type\":\"COSINE\"}}" >/dev/null || true
  ...
  curl -fsS -X PUT "${NEXT_PUBLIC_API_URL}/api/v1/config/mem0/vector_store" \
    -H 'Content-Type: application/json' \
    -d "{\"provider\":\"weaviate\",\"config\":{\"collection_name\":\"memverse\",\"embedding_model_dims\":${EMBEDDING_DIMS},\"cluster_url\":\"http://memverse_store:8080\"}}" >/dev/null || true
  ...
  curl -fsS -X PUT "${NEXT_PUBLIC_API_URL}/api/v1/config/mem0/vector_store" \
    -H 'Content-Type: application/json' \
    -d "{\"provider\":\"redis\",\"config\":{\"collection_name\":\"memverse\",\"embedding_model_dims\":${EMBEDDING_DIMS},\"redis_url\":\"redis://memverse_store:6379\"}}" >/dev/null || true
  ...
  curl -fsS -X PUT "${NEXT_PUBLIC_API_URL}/api/v1/config/mem0/vector_store" \
    -H 'Content-Type: application/json' \
    -d "{\"provider\":\"pgvector\",\"config\":{\"collection_name\":\"memverse\",\"embedding_model_dims\":${EMBEDDING_DIMS},\"dbname\":\"mem0\",\"user\":\"mem0\",\"password\":\"mem0\",\"host\":\"memverse_store\",\"port\":5432,\"diskann\":false,\"hnsw\":true}}" >/dev/null || true
  ...
  curl -fsS -X PUT "${NEXT_PUBLIC_API_URL}/api/v1/config/mem0/vector_store" \
    -H 'Content-Type: application/json' \
    -d "{\"provider\":\"qdrant\",\"config\":{\"collection_name\":\"memverse\",\"embedding_model_dims\":${EMBEDDING_DIMS},\"host\":\"memverse_store\",\"port\":6333}}" >/dev/null || true
  ...
  curl -fsS -X PUT "${NEXT_PUBLIC_API_URL}/api/v1/config/mem0/vector_store" \
    -H 'Content-Type: application/json' \
    -d "{\"provider\":\"chroma\",\"config\":{\"collection_name\":\"memverse\",\"host\":\"memverse_store\",\"port\":8000}}" >/dev/null || true
  ...
  curl -fsS -X PUT "${NEXT_PUBLIC_API_URL}/api/v1/config/mem0/vector_store" \
    -H 'Content-Type: application/json' \
    -d "{\"provider\":\"elasticsearch\",\"config\":{\"collection_name\":\"memverse\",\"embedding_model_dims\":${EMBEDDING_DIMS},\"host\":\"http://memverse_store\",\"port\":9200,\"user\":\"elastic\",\"password\":\"changeme\",\"verify_certs\":false,\"use_ssl\":false}}" >/dev/null || true
  ...
  curl -fsS -X PUT "${NEXT_PUBLIC_API_URL}/api/v1/config/mem0/vector_store" \
    -H 'Content-Type: application/json' \
    -d "{\"provider\":\"faiss\",\"config\":{\"collection_name\":\"memverse\",\"embedding_model_dims\":${EMBEDDING_DIMS},\"path\":\"/tmp/faiss\",\"distance_strategy\":\"cosine\"}}" >/dev/null || true
fi
