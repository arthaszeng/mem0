...
class TestConfidenceThresholdConfig:
    def test_config_schema_accepts_threshold(self):
        from app.routers.config import OpenMemoryConfig
        config = OpenMemoryConfig(confidence_threshold=0.7)
    ...
    def test_config_schema_rejects_invalid_threshold(self):
        ...
        import pydantic
        try:
            OpenMemoryConfig(confidence_threshold=1.5)
    ...
    def test_get_config_includes_threshold(self, client):
        response = client.get("/api/v1/config/openmemory")
